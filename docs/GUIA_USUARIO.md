# 🥛 Gloria Lácteos - Guía de Usuario

## Bienvenido

Gloria Lácteos es una tienda online de productos lácteos donde puedes comprar los mejores productos Gloria del Perú.

---

## 🔑 Cómo Ingresar

### Registro
1. Haz clic en **"📝 Registrarse"** en el menú superior
2. Ingresa tu nombre, email y contraseña
3. Haz clic en **"Crear cuenta"**

### Iniciar Sesión
1. Haz clic en **"🔑 Ingresar"**
2. Ingresa tu email y contraseña
3. Haz clic en **"Ingresar"**

---

## 🛒 Cómo Comprar

### Agregar productos al carrito
1. Navega por los productos en la tienda
2. Haz clic en **"🛒 Agregar al carrito"** en el producto que desees
3. El contador del carrito aumentará

### Ver tu carrito
1. Haz clic en **"🛒 Carrito"** en el menú superior
2. Verás todos los productos agregados con:
   - Cantidad (puedes modificar con ➕ y ➖)
   - Stock disponible
   - Subtotal por producto
   - Total de la compra

### Finalizar compra
1. En el carrito, haz clic en **"🎉 Comprar ahora"**
2. Aparecerá un mensaje confirmando tu compra con el **número de orden**
3. El stock de los productos se actualizará automáticamente
4. Tu orden quedará registrada con estado **"Pendiente"**

---

## 📦 Estados de tu Pedido

| Estado | Icono | Significado |
|--------|-------|-------------|
| Pendiente | ⏳ | Orden recibida, en espera de procesamiento |
| Enviado | 🚚 | Tu pedido está en camino |
| Entregado | ✅ | Pedido completado exitosamente |

---

## 👤 Tu Perfil

### Editar perfil
1. Haz clic en tu nombre en el menú superior
2. Puedes cambiar tu nombre y contraseña
3. Haz clic en **"Guardar cambios"**

---

## ⚠️ Estados de Productos

| Estado | Significado |
|--------|-------------|
| ✓ X unidades disponibles | Producto disponible para comprar |
| ❌ Sin stock | Producto agotado |
| ⚠️ VENCIDO | Producto con lote vencido, no disponible |

Los productos agotados o vencidos aparecen en **gris** y no se pueden agregar al carrito.

---

## 💾 Tu Carrito se Guarda

Si cierras sesión, tu carrito se guarda automáticamente. Cuando vuelvas a iniciar sesión, encontrarás tus productos esperándote.

---

## 📞 Contacto

Gloria Lácteos - Tienda Online  
Hecho con ❤️ en Perú
